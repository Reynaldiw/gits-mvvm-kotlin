...
object GitsRemoteDataSource : GitsDataSource {
    ...
    private val teamListAdapter : Gson by lazy {
        GsonBuilder()
                .registerTypeAdapter(Team::class.java, GitsNullAdapter())
                .create()
    }

    override fun getTeams(callback: GitsDataSource.GetTeamsCallback) {
        GitsApiService.getApiService
                ...
                .subscribe(object : ApiCallback<BaseApiModel<List<Team>>>() {
                    override fun onSuccess(model: BaseApiModel<List<Team>>) {
                        val oldData = model.teams

                        callback.onSuccess(oldData ?: emptyList())
                    }
